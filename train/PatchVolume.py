...
class patchvolumeAE(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        x = batch['data']
        opts = self.optimizers() if callable(self.optimizers) else self.optimizers
        ...
        if self.stage == 1 :
            if optimizer_idx == 0:
                recon_loss, x_recon, vq_output, aeloss, perceptual_loss, gan_feat_loss = self.forward(
                    x, optimizer_idx)
                ...
                log_dict.update({
                    ...
                    'train/commitment_loss': commitment_loss.detach(),
                    'train/aeloss': aeloss.detach(),
                    'train/perceptual_loss': perceptual_loss.detach(),
                    'train/gan_feat_loss': gan_feat_loss.detach(),
                    'train/perplexity': vq_output['perplexity'].detach(),
            ...
            if optimizer_idx == 1:
                ...
                # 收集判别器相关的日志
                log_dict.update({
                    'train/discloss': discloss.detach(),
                    'optimizer_idx': 1
                })
            ...
            # 记录重建样本（每500步记录一次）
            if optimizer_idx == 0 and self.global_step % 500 == 0:
                self._log_reconstruction_samples(x, x_recon, log_dict)
    ...
    def _log_to_wandb(self, log_dict):
        """将日志记录到wandb"""
        try:
            if hasattr(self.logger, 'experiment') and hasattr(self.logger.experiment, 'log'):
                ...
                log_dict.update({
                    ...
                    'stage': self.stage,
                    'batch_size': self.trainer.world_size * self.cfg.model.batch_size
                })
    ...
    def _get_gradient_stats(self):
        ...
        for name, param in self.named_parameters():
            if param.grad is not None:
                ...
                # 只记录主要层的梯度信息，避免日志过多
                if any(layer_name in name.lower() for layer_name in ['encoder', 'decoder', 'discriminator']):
                    grad_stats[f'grad_norm/{name}'] = grad_norm
                    grad_stats[f'grad_mean/{name}'] = grad_mean
                    grad_stats[f'grad_std/{name}'] = grad_std
                
                total_grad_norm += grad_norm ** 2
                param_count += 1
        
        if param_count > 0:
            grad_stats['grad_norm/total'] = (total_grad_norm / param_count) ** 0.5
        
        return grad_stats

    def _log_reconstruction_samples(self, x, x_recon, log_dict):
        """记录重建样本到wandb"""
        try:
            import wandb
            import torchvision
            ...
            # 提取中间切片
            x_slices = x[:num_samples, :, mid_slice, :, :]
            x_recon_slices = x_recon[:num_samples, :, mid_slice, :, :]
            
            # 归一化到[0, 1]范围
            x_slices = (x_slices + 1) / 2  # 从[-1, 1]到[0, 1]
            x_recon_slices = (x_recon_slices + 1) / 2
            
            # 创建对比图像
            comparison_images = []
            for i in range(num_samples):
                # 原始图像
                orig_img = x_slices[i].unsqueeze(0)  # [1, 1, H, W]
                # 重建图像
                recon_img = x_recon_slices[i].unsqueeze(0)  # [1, 1, H, W]
                # 拼接
                comparison = torch.cat([orig_img, recon_img], dim=3)  # [1, 1, H, 2*W]
                comparison_images.append(comparison)
            
            # 创建网格
            if comparison_images:
                grid = torchvision.utils.make_grid(
                    torch.cat(comparison_images, dim=0), 
                    nrow=1, 
                    padding=2,
                    normalize=False
                )
                ...
                # 添加到日志字典
                log_dict['reconstruction_samples'] = wandb.Image(grid, caption="Original | Reconstructed")
                
                # 计算并记录PSNR和SSIM
                psnr = self._calculate_psnr(x[:num_samples], x_recon[:num_samples])
                ssim = self._calculate_ssim(x[:num_samples], x_recon[:num_samples])
                
                log_dict['metrics/psnr'] = psnr
                log_dict['metrics/ssim'] = ssim
                
        except Exception as e:
            print(f"Error logging reconstruction samples: {e}")

    def _calculate_psnr(self, x, x_recon):
        """计算PSNR"""
    ...
    def _calculate_ssim(self, x, x_recon):
        """计算SSIM的简化版本"""
        # 这里使用简化的SSIM计算，实际项目中可以使用更精确的实现
        mean_x = x.mean()
        mean_recon = x_recon.mean()
        std_x = x.std()
        std_recon = x_recon.std()
        cov = ((x - mean_x) * (x_recon - mean_recon)).mean()
        
        c1 = (0.01 * 2) ** 2
        c2 = (0.03 * 2) ** 2
        
        numerator = (2 * mean_x * mean_recon + c1) * (2 * cov + c2)
        denominator = (mean_x**2 + mean_recon**2 + c1) * (std_x**2 + std_recon**2 + c2)
